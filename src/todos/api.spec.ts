...
import { PREFIX, Todo, CreateTodo, UpdateTodo } from './model'

const env = getMiniflareBindings()
const TEST_URL = 'http://localhost/api/v1/todos'
...
describe('Todos API', () => {
  ...
  test('Todo 一覧を取得する', async () => {
    const res = await app.fetch(new Request(TEST_URL), env)
  ...
  test('Todo を作成する', async () => {
    ...
    const res2 = await app.fetch(new Request(TEST_URL), env)
  ...
  test('Todo を作成する：title は必須', async () => {
    const newTodo: CreateTodo = { title: 'Sell Pokemon' }
